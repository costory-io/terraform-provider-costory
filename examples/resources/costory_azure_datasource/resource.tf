...
# ──────────────────────────────────────────────
# Cost Management Exports (Parquet, Daily)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Backfill (auto last N months via /run action)
# ──────────────────────────────────────────────
...
locals {
  ...
  backfill_ranges = {
    for m in local.backfill_months : m => {
      from = "${m}-01T00:00:00Z"
      to = "${formatdate("YYYY-MM-DD", timeadd(
        format("%04d-%02d-01T00:00:00Z",
...
resource "azapi_resource_action" "backfill_actuals" {
  for_each   = local.backfill_ranges
  type       = "Microsoft.CostManagement/exports@2025-03-01"
  resource_id = azapi_resource.actuals.id
  action     = "run"
  method     = "POST"
  when       = "apply"
  body       = { timePeriod = { from = each.value.from, to = each.value.to } }
}
...
data "azurerm_storage_account_sas" "billing" {
  ...
  resource_types    = { service = false, container = true, object = true }
